...
export interface HttpApi {
     ...
     */
    get<T = MarshalableType, P = Record<string, any> | void>(url: string, params?: P, timeout?: number): Promise<T>;
    /**
     * POST 메서드 호출
     ...
     * @param timeout 응답 제한시간 설정. 설정한 시간이 넘도록 응답이 없다면 중단하고 throw 를 일으킨다.
     */
    post<T = MarshalableType, P = Record<string, any> | void>(url: string, body?: P, timeout?: number): Promise<T>;
    /**
     * PUT 메서드 호출
     * @param url 호출 경로. 만약 쿼리 파라미터가 포함된다면 이 곳에 추가적으로 명시 해 주어야 한다.
     * @param body 요청으로 보낼 데이터. json 으로 바꿔서 보내게 된다.
     * @param timeout 응답 제한시간 설정. 설정한 시간이 넘도록 응답이 없다면 중단하고 throw 를 일으킨다.
     */
    put<T = MarshalableType, P = Record<string, any> | void>(url: string, body?: P, timeout?: number): Promise<T>;
    /**
     * PATCH 메서드 호출
     * @param url 호출 경로. 만약 쿼리 파라미터가 포함된다면 이 곳에 추가적으로 명시 해 주어야 한다.
     * @param body 요청으로 보낼 데이터. json 으로 바꿔서 보내게 된다.
     * @param timeout 응답 제한시간 설정. 설정한 시간이 넘도록 응답이 없다면 중단하고 throw 를 일으킨다.
     */
    patch<T = MarshalableType, P = Record<string, any> | void>(url: string, body?: P, timeout?: number): Promise<T>;
    /**
     * DELETE 메서드 호출
     * @param url 호출 경로
     * @param params 전달할 파라미터
     * @param timeout 응답 제한시간 설정. 설정한 시간이 넘도록 응답이 없다면 중단하고 throw 를 일으킨다.
     */
    delete<T = MarshalableType, P = Record<string, any> | void>(url: string, params?: P, timeout?: number): Promise<T>;
    /**
     * POST 메서드로 업로드 한다.
     * @param url 업로드 경로
